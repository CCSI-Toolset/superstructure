...
$CALL  gdxxrw.exe C:\Users\mzamarripa\Documents\Superstructure_project\01_Project_files\SurrogateModels\Regenerator\BUF_surrogates\RGN_BUF_Variables.xlsx @C:\Users\mzamarripa\Documents\Superstructure_project\01_Project_files\SurrogateModels\Regenerator\BUF_surrogates\surrogate_models\parameters_RGN_BUF.txt
$GDXIN RGN_BUF_Variables.gdx
$LOAD i
...
*GasOutFlow
*+xf('3')* (- 7079.6472712 * vgValue0(i) + 2122.0468034 * (Dt(i)) * vgValue0(i) )
...
*SolidOutH2O
*( - 0.231188E-001 * unitD(d)/3.28084 * ( solidOutC(d+1,'H2O')$(ORD(d) ne CARD(d)) + solidOutC('a1','H2O')$(ORD(d) eq CARD(d)) ) + 0.3044147 * ( SolidoutC(d+1,'NH2COO')$(ORD(d) ne Card(d))+ SolidoutC('a1','NH2COO')$(ORD(d) eq Card(d)) )* (solidOutC(d+1,'H2O')$(ORD(d) ne CARD(d)) + solidOutC('a1','H2O')$(ORD(d) eq CARD(d)) ) )*x(d,'BUF')
*+xf('6')*( - 0.231188E-001 * Dt(i) * ( SolidInH2O(i) ) + 0.3044147 * ( SolidInCar(i) )* (SolidInH2O(i)) )
+xf('6')* (  - (a1('6') * Dt(i) * solidInH2O(i) ) + a2('6') * (SolidInCar(i)**a('6')) * solidInH2O(i)**b('6')    )
...
a1.l('3')=2293; a2.l('3')=58.37; a.l('3')=1; b.l('3')=1; c.l('3')=1;
...
a.l('6')=1 ;b.l('6')=1; a1.l('6')=0.231188E-001; a2.l('6')= 0.3044147;
...
dim=1; dim=1000000$(ord(Z) lt 5);
         loop(i,
         estim(i)=
         +xf('1')*Nx(i) +xf('2')*GasOutP(i) +xf('3')*GasOutF(i) +xf('4')*GasOutT(i) +xf('5')*SolidOutT(i) +xf('6')*SolidOutH2O(i)
         +xf('7')*SolidOutBic(i) +xf('8')*SolidOutCar(i) +xf('9')*GasOutCO2(i) +xf('10')*GasOutH2O(i) +xf('11')*HxinT(i) );
         display xf, estim,xx;
...
Execute_unload "RGN_BUF_Results.gdx" ParR2, Parminv, Parmaxv, ParA, ParB, ParC, ParD, ParA1, ParA2, ParA3, ParA4, time;
...
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParR2.l rng=R2!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=Parminv.l rng=Parminv!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=Parmaxv.l rng=Parmaxv!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParA.l rng=ParA!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParB.l rng=ParB!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParC.l rng=ParC!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParD.l rng=ParD!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParA1.l rng=ParA1!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParA2.l rng=ParA2!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParA3.l rng=ParA3!'
Execute 'gdxxrw.exe RGN_BUF_Results.gdx Squeeze=N par=ParA4.l rng=ParA4!'
